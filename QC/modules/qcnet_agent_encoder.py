...
class QCNetAgentEncoder(nn.Module):
    ...
    def forward(self,
        ...
        motion_vector_a = torch.cat([pos_a.new_zeros(data['agent']['num_nodes'], 1, self.input_dim), #agent_num_nodes，1，2
                                     pos_a[:, 1:] - pos_a[:, :-1]], dim=1)  #pos_a[:, 1:] - pos_a[:, :-1] （agent_num_nodes，num_historical_steps-1，2）
        ##cat 起来之后就是（agent_num_nodes，num_historical_steps，2）
